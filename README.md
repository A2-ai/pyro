...
1.  **Seed (first init only).** If the project has no `pyproject.toml`,
    ...
    spec (`inst/extdata/pyproject.toml`). The seed is templated with the
...
- **`pyproject_dir`** lets non-default project layouts (LLM agents,
  monorepos, closed-source projects with a secret toml elsewhere) point
  `pyro` at an alternate location. Must already exist; `pyro`
  does not create project directories.
- **`getOption("venv_dir")`** caches the resolved project root after the
  first init; later calls in the same R session skip re-resolution.
