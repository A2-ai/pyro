test_that("get_venv_uv_paths sets venv_dir if unset", {
  withr::local_options(list(venv_dir = NULL))

  temp_dir <- tempfile()
  dir.create(file.path(temp_dir, ".venv"), recursive = TRUE)

  mockery::stub(get_venv_uv_paths, "get_proj_dir", function() temp_dir)
...
test_that("get_venv_uv_paths throws error when .venv directory is missing", {
  temp_dir <- tempfile()
...
test_that("get_venv_uv_paths throws error when uv path is NULL", {
  temp_dir <- tempfile()
...
test_that("get_venv_uv_paths returns correct paths", {
  temp_dir <- tempfile()
  venv_path <- file.path(temp_dir, ".venv")
  dir.create(venv_path, recursive = TRUE)
